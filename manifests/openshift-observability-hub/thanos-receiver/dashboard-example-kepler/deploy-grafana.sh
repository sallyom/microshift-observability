...
while ! oc get serviceaccount $SERVICE_ACCOUNT -n thanos
do
    sleep 2
done
# Define Prometheus datasource
oc adm policy add-cluster-role-to-user cluster-monitoring-view -z $SERVICE_ACCOUNT -n thanos


export BEARER_TOKEN=$(oc get secret ${SECRET} -o json -n thanos | jq -Mr '.data.token' | base64 -d) || or true
